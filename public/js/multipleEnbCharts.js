...
function createPieChart(canvasId, label, data, total) {
  ...
  new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: [`Antenas identificadas (${data})`, `Por identificar (${total - data})`],
      datasets: [{
...
async function generateCharts() {
  ...
  for (const operadora of operadoras) {
    try {
      ...
      createPieChart(`${operadora.toLowerCase()}Canvas`, operadoraNames[operadora], operadoraENBCount, percentuais[operadora]);
      console.log(`Created chart for ${operadoraNames[operadora]}`);
    } catch (error) {
      console.error(`Error generating chart for ${operadoraNames[operadora]}:`, error);
    }
  }
}
